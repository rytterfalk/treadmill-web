#!/usr/bin/env python3
import os, asyncio, aiosqlite, json, logging, re
...
total = 0
async with db.execute("SELECT result_json FROM progressive_program_days ppd JOIN progressive_programs pp ON pp.id = ppd.program_id WHERE pp.user_id = ? AND ppd.date = ?", (userid, target_date)) as cur:
...
async with db.execute("SELECT id, session_type, started_at, duration_sec, treadmill_state_json, notes FROM workout_sessions WHERE user_id = ? AND date(started_at) = date(?) ORDER BY started_at ASC", (userid, target_date)) as cur:
rows = await cur.fetchall()
for row in rows:
...
if g:
total_reps += int(g)
# also "10x3" patterns? ignore for now
return total_reps, total_minutes
async def sum_circuit_for_date(db, userid=1, target_date=None):
if target_date is None:
target_date = date.today().isoformat()
total_reps = 0
async with db.execute("SELECT exercise_times FROM circuit_sessions WHERE user_id = ? AND date(completed_at) = date(?)", (userid, target_date)) as cur:
rows = await cur.fetchall()
for (etxt,) in rows:
...
rows = []
async with db.execute("SELECT id, user_id, session_type, started_at, duration_sec, treadmill_state_json, notes FROM workout_sessions WHERE COALESCE(started_at, created_at) >= datetime('now','-7 days') ORDER BY COALESCE(started_at, created_at) ASC") as cur:
    ...
    try:
        ...
        async with aiosqlite.connect(DBPATH) as db:
            ...
            for r in new_rows:
                ...
                # also check free-text replies saved elsewhere: we will parse memory files later
            ...
            async with db.execute("SELECT COALESCE(started_at, created_at) FROM workout_sessions WHERE user_id = ? ORDER BY COALESCE(started_at, created_at) DESC LIMIT 1", (userid,)) as cur:
            ...
            memdir = os.path.expanduser("~/memory")
